...
public abstract class AbstractJobConfigGsonTypeAdapter<T extends JobRootConfiguration> extends TypeAdapter<T> {
	...
	private JobCoreConfiguration getJobCoreConfiguration(final String jobName, final String cron,
		...
		return JobCoreConfiguration
				...
				.segmentItemParameters(shardingItemParameters)
	...
	public void write(final JsonWriter out, final T value) throws IOException {
		...
		out.name("shardingTotalCount").value(value.getTypeConfig().getCoreConfig().getSegmentTotalCount());
		out.name("shardingItemParameters").value(value.getTypeConfig().getCoreConfig().getSegmentItemParameters());
		...
		if (value.getTypeConfig().getJobType() == JobType.FLOW) {
			FlowJobConfiguration dataflowJobConfig = (FlowJobConfiguration) value.getTypeConfig();

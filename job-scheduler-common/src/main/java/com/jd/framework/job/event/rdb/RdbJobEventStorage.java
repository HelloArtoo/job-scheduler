...
import static com.jd.framework.job.constant.rdb.JobTable.INDEX_TASK_ID_STATE;
...
import com.jd.framework.job.constant.rdb.Database;
 ...
 */
@Slf4j
public class RdbJobEventStorage {
	...
	private void initTablesAndIndexes() throws SQLException {
		try (Connection conn = dataSource.getConnection()) {
			...
			database = Database.valueFrom(conn.getMetaData().getDatabaseProductName());
		}
	}

	private void createJobExecutionTableAndIndexIfNeeded(final Connection conn) throws SQLException {
		DatabaseMetaData dbMetaData = conn.getMetaData();
		try (ResultSet resultSet = dbMetaData.getTables(null, null, TABLE_LOG_JOB_EXECUTION, new String[] { "TABLE" })) {
			if (!resultSet.next()) {
	...
	private void createJobStatusTraceTableAndIndexIfNeeded(final Connection conn) throws SQLException {
		DatabaseMetaData dbMetaData = conn.getMetaData();
		try (ResultSet resultSet = dbMetaData.getTables(null, null, TABLE_LOG_JOB_STATUS_TRACE,
				new String[] { "TABLE" })) {
			if (!resultSet.next()) {
				createJobStatusTraceTable(conn);
			}
		}
		createTaskIdIndexIfNeeded(conn, TABLE_LOG_JOB_STATUS_TRACE, INDEX_TASK_ID_STATE);
	}
	...
	private void createJobExecutionTable(final Connection conn) throws SQLException {
		String dbSchema = "CREATE TABLE `" + TABLE_LOG_JOB_EXECUTION + "` (" + "`id` VARCHAR(40) NOT NULL, "
				+ "`job_name` VARCHAR(100) NOT NULL, " + "`task_id` VARCHAR(255) NOT NULL, "
				+ "`hostname` VARCHAR(255) NOT NULL, " + "`ip` VARCHAR(50) NOT NULL, "
				+ "`segment_item` INT NOT NULL, " + "`execution_source` VARCHAR(20) NOT NULL, "
				+ "`failure_cause` VARCHAR(4000) NULL, " + "`is_success` INT NOT NULL, "
				+ "`start_time` TIMESTAMP NULL, " + "`complete_time` TIMESTAMP NULL, " + "PRIMARY KEY (`id`));";
	...
	private void createJobStatusTraceTable(final Connection conn) throws SQLException {
		String dbSchema = "CREATE TABLE `" + TABLE_LOG_JOB_STATUS_TRACE + "` (" + "`id` VARCHAR(40) NOT NULL, "
				...
				+ "`segment_item` VARCHAR(100) NOT NULL, " + "`state` VARCHAR(20) NOT NULL, "
				+ "`message` VARCHAR(4000) NULL, " + "`creation_time` TIMESTAMP NULL, " + "PRIMARY KEY (`id`));";
		try (PreparedStatement preparedStatement = conn.prepareStatement(dbSchema)) {
			preparedStatement.execute();
		}
	}
	...
	private void createTaskIdAndStateIndex(final Connection conn, final String tableName) throws SQLException {
		String sql = "CREATE INDEX " + INDEX_TASK_ID_STATE + " ON " + tableName + " (`task_id`, `state`);";
	...
	private boolean insertJobExecutionEvent(final JobExecutionEvent jobExecutionEvent) {
		...
		String sql = "INSERT INTO `"
				+ TABLE_LOG_JOB_EXECUTION
				+ "` (`id`, `job_name`, `task_id`, `hostname`, `ip`, `segment_item`, `execution_source`, `is_success`, `start_time`) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);";
		try (Connection conn = dataSource.getConnection();
			...
			preparedStatement.setInt(6, jobExecutionEvent.getSegmentItem());
	...
	private boolean isDuplicateRecord(final SQLException ex) {
		return Database.MySQL.equals(database) && 1062 == ex.getErrorCode() || Database.H2.equals(database)
				&& 23505 == ex.getErrorCode() || Database.SQLServer.equals(database) && 1 == ex.getErrorCode()
				|| Database.DB2.equals(database) && -803 == ex.getErrorCode() || Database.PostgreSQL.equals(database)
				&& 0 == ex.getErrorCode() || Database.Oracle.equals(database) && 1 == ex.getErrorCode();
	}

	private boolean updateJobExecutionEventWhenSuccess(final JobExecutionEvent jobExecutionEvent) {
		boolean result = false;
		String sql = "UPDATE `" + TABLE_LOG_JOB_EXECUTION + "` SET `is_success` = ?, `complete_time` = ? WHERE id = ?";
		try (Connection conn = dataSource.getConnection();
				PreparedStatement preparedStatement = conn.prepareStatement(sql)) {
	...
	private boolean insertJobExecutionEventWhenSuccess(final JobExecutionEvent jobExecutionEvent) {
		...
		String sql = "INSERT INTO `"
				+ TABLE_LOG_JOB_EXECUTION
				+ "` (`id`, `job_name`, `task_id`, `hostname`, `ip`, `segment_item`, `execution_source`, `is_success`, `start_time`, `complete_time`) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
		try (Connection conn = dataSource.getConnection();
				PreparedStatement preparedStatement = conn.prepareStatement(sql)) {
			preparedStatement.setString(1, jobExecutionEvent.getId());
	...
	private boolean updateJobExecutionEventFailure(final JobExecutionEvent jobExecutionEvent) {
		boolean result = false;
		String sql = "UPDATE `" + TABLE_LOG_JOB_EXECUTION + "` SET `is_success` = ?, `failure_cause` = ? WHERE id = ?";
		try (Connection conn = dataSource.getConnection();
				PreparedStatement preparedStatement = conn.prepareStatement(sql)) {
			preparedStatement.setBoolean(1, jobExecutionEvent.isSuccess());
	...
	private boolean insertJobExecutionEventWhenFailure(final JobExecutionEvent jobExecutionEvent) {
		...
		String sql = "INSERT INTO `"
				+ TABLE_LOG_JOB_EXECUTION
				+ "` (`id`, `job_name`, `task_id`, `hostname`, `ip`, `segment_item`, `execution_source`, `failure_cause`, `is_success`, `start_time`) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
		try (Connection conn = dataSource.getConnection();
				PreparedStatement preparedStatement = conn.prepareStatement(sql)) {
			preparedStatement.setString(1, jobExecutionEvent.getId());
	...
	boolean addJobStatusTraceEvent(final JobStatusTraceEvent jobStatusTraceEvent) {
		...
		boolean result = false;
		String sql = "INSERT INTO `"
				+ TABLE_LOG_JOB_STATUS_TRACE
				+ "` (`id`, `job_name`, `original_task_id`, `task_id`, `slave_id`, `source`, `execution_type`, `segment_item`,  "
		...
		try (Connection conn = dataSource.getConnection();
			...
			preparedStatement.setString(8, jobStatusTraceEvent.getSegmentItems());
	...
	private String getOriginalTaskId(final String taskId) {
		String sql = String.format("SELECT original_task_id FROM %s WHERE task_id = '%s' and state='%s'",
				TABLE_LOG_JOB_STATUS_TRACE, taskId, State.TASK_STAGING);
		String result = "";
		try (Connection conn = dataSource.getConnection();
				PreparedStatement preparedStatement = conn.prepareStatement(sql);
	...
	List<JobStatusTraceEvent> getJobStatusTraceEvents(final String taskId) {
		String sql = String.format("SELECT * FROM %s WHERE task_id = '%s'", TABLE_LOG_JOB_STATUS_TRACE, taskId);
